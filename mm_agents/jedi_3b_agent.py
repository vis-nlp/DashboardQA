...
JEDI_API_KEY = "abc123"
JEDI_SERVICE_URL = "http://127.0.0.1:8000"
...
class JediAgent3B:
    ...
    def convert_action_to_grounding_model_instruction(self, line: str, obs: Dict, instruction: str, height: int, width: int, resized_height: int, resized_width: int ) -> str:
        ...
        for match in matches:
            ...
            ## Custom Model Name
            call_llm_executor_model = "xlangai/Jedi-3B-1080p"
            #### 
            grounding_response = self.call_llm({
                "model": call_llm_executor_model, #self.executor_model,
                "messages": messages,
                "max_tokens": self.max_tokens,
                "top_p": self.top_p,
                "temperature": self.temperature
            }, self.executor_model)
            logger.info(grounding_response)
            logger.info(f"{height}, {width}")
            logger.info(f"{resized_height}, {resized_width}")
