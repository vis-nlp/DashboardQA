...
logger = logging.getLogger("desktopenv.jedi_3b_agent")

GTA1_API_KEY = "abc123" #"Your GTA1 API Key"
GTA1_MODEL_NMAE = "HelloKKMe/GTA1-7B"  #Your served model name
GTA1_SERVICE_URL = "http://0.0.0.0:8000" #os.environ.get("GTA1_SERVICE_URL",None) #"Your GTA1 Service URL"
    ...
    def predict(self, instruction: str, obs: Dict) -> List:
        ...
        while N > 0: 
            ...
            for bn in split_to_batches(N, batch_size=8):
                ...
                planner_response.extend(planner_response_)
                logger.info(f"Retry Planner Output: {planner_response_}")
        ...
        logger.info("Parsed Code: %s", str(codes))
        logger.info("Parsed Thought: %s", str(thought))
        logger.info("Parsed Observation: %s", str(observation_caption))
        ...
        def request_vllm(image, prompt):
            ...
            vllm_client = OpenAI(
                base_url=f"{GTA1_SERVICE_URL}/v1", 
