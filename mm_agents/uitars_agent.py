...
class UITARSAgent:
    ...
    def predict(
        ...
        # These variables are not used a nywhere. 
        ...
        if len(self.history_responses) > 0:
            for history_idx, history_response in enumerate(self.history_responses):
                ...
                messages.append({
                    "role": "assistant",
                    "content": [{"type": "text", "text": add_box_token(history_response)}], #[add_box_token(history_response)]
                })
        ...
        while True:
            ...
            logger.info("Generating content with GPT model: ByteDance-Seed/UI-TARS-1.5-7B")
            try:
                response = self.vlm.chat.completions.create(
                    model="ByteDance-Seed/UI-TARS-1.5-7B", #"ahmed-masry/UI-TARS-2B-SFT", #"ui-tars",
                    messages=messages,
                    frequency_penalty=1,
            ...
            if parsed_responses:
                logger.info("PARSED RESPONSE: %s", parsed_responses)  
        ...
        if parsed_responses:
            logger.info("FINAL PARSED RESPONSE: %s", parsed_responses)  
        actions = []
        ...
        for parsed_response in parsed_responses:
            ...
            logger.info("PYAUTOCODE: %s", pyautogui_code) 
