...
from mm_agents.prompts import SYS_PROMPT_IN_SCREENSHOT_OUT_CODE, SYS_PROMPT_IN_SCREENSHOT_OUT_ACTION, \
    ...
    SYS_PROMPT_IN_SOM_OUT_TAG, SYS_PROMPT_IN_SOM_OCR_OUT_TAG
...
def get_bbox_coords_and_size(vertices):
    """
    Given a list of 4 vertices, returns [top_left_x, top_left_y] and [width, height].
    Assumes vertices are in order: top-left, top-right, bottom-right, bottom-left.
    """
...
def extract_lines_and_words_lists(ocr_response):
    """
    Returns:
        lines: list of dicts, each {"text": str, "bounding_box": {"coords": [x, y], "size": [w, h]}}
        words: list of dicts, each {"text": str, "bounding_box": {"coords": [x, y], "size": [w, h]}}
    """
    lines = []
    words = []
    try:
        for page in ocr_response.full_text_annotation.pages:
            for block in page.blocks:
                for para in block.paragraphs:
                    # Line/Paragraph
                    para_box = para.bounding_box.vertices
                    coords, size = get_bbox_coords_and_size(para_box)
                    line_text = ""
                    line_words = []
                    for word in para.words:
                        word_text = "".join([symbol.text for symbol in word.symbols])
                        word_box = word.bounding_box.vertices
                        w_coords, w_size = get_bbox_coords_and_size(word_box)
                        word_entry = {
                            "text": word_text,
                            "bounding_box": {
                                "coords": w_coords,
        ...
        print("Error extracting lines and words:", e)
    return lines, words
...
class PromptAgent:
    def __init__(
            ...
            # observation_type can be in ["screenshot", "a11y_tree", "screenshot_a11y_tree", "som", "som_ocr"]
        ...
        if observation_type == "screenshot":
            ...
            if action_space == "computer_13":
                ...
                raise ValueError("Invalid action space: " + action_space)
        elif observation_type == "som_ocr": 
            if action_space == "computer_13":
                raise ValueError("Invalid action space: " + action_space)
            elif action_space == "pyautogui":
                self.system_message = SYS_PROMPT_IN_SOM_OCR_OUT_TAG
            else:
                raise ValueError("Invalid action space: " + action_space)
        else:
            raise ValueError("Invalid experiment type: " + observation_type)

    def detect_ocr(self, image_data) -> List:
        # Convert the image data to bytes for Google Vision API
        ...
        ocr_response = self.vision_client.document_text_detection(image=vision_image)
        lines, words = extract_lines_and_words_lists(ocr_response)
        # For now, we will use words
        return words
    ...
    def predict(self, instruction: str, obs: Dict) -> List:
        ...
        for previous_obs, previous_action, previous_thought in zip(_observations, _actions, _thoughts):
            ...
            elif self.observation_type in ["som", "som_ocr"]:
                _screenshot = previous_obs["screenshot"]
        ...
        elif self.observation_type == "som_ocr":
            # Add som to the screenshot
            ocr_list = self.detect_ocr(obs["screenshot"])
            masks, drew_nodes, tagged_screenshot, linearized_accessibility_tree = tag_screenshot(obs["screenshot"], obs[
                "accessibility_tree"], self.platform)
            base64_image = encode_image(tagged_screenshot)
            logger.debug("LINEAR AT: %s", linearized_accessibility_tree)
    ...
    def call_llm(self, payload):
        ...
        elif self.model.startswith("o4-mini"): # Added O4-Mini
            headers = {
                "Content-Type": "application/json",
                "Authorization": f"Bearer {os.environ['OPENAI_API_KEY']}"
            }
